...
module.exports = {
  async create(ctx) {
    try {
      ...
      const sanitizedData = {
        client_title: title ? validator.escape(title) : '',
        name: full_name ? validator.escape(full_name) : '',
        phone_number: phone ? validator.trim(phone) : '',
        email: email ? (validator.normalizeEmail(email) || '') : '',
        adults: parseInt(adults, 10) || 1,
        children: parseInt(children, 10) || 0,
        infants: parseInt(infants, 10) || 0,
        departure_date: travel_date ? validator.toDate(travel_date) : null,
        depart_from: city ? validator.escape(city) : '',
        destination: destination ? validator.escape(destination) : '',
        return_date: return_date ? validator.toDate(return_date) : null,
        trip_type: tripType ? validator.escape(tripType) : '',
